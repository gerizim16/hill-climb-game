...
class Window(pyglet.window.Window):
    ...
    def update(self, dt):
        ...
        if new_mode:
            ...
            print(
                "ID: {} | args: {} | kwargs: {}".format(
                    new_mode, args_to_pass, kwargs_to_pass
                )
            )
            ...
            self.space.remove(
                *self.space.bodies, *self.space.shapes, *self.space.constraints
            )
            del self.space
            self.space = pymunk.Space()
            ...
            if new_mode == Game3.id:
                ...
                self.bg_music.volume = 0.6
                self.activated_mode = Endgame(
                    self.main_batch, self.space, self, *args_to_pass, **kwargs_to_pass
                )
            elif new_mode == HighScore.id:
                self.bg_music.volume = 0.6
                self.activated_mode = HighScore(
                    self.main_batch, self.space, self, *args_to_pass, **kwargs_to_pass
                )
...
class GameState(object):
    ...
    def __init__(self, batch, space, window, mouse_hover=False, bounded=False):
        ...
        if bounded:
            ...
            left_bound_s = pymunk.Segment(
                bounds_body, (-20, 0), (-20, self.window.height), 20
            )
            right_bound_s = pymunk.Segment(
                bounds_body,
                (self.window.width + 20, 0),
                (self.window.width + 20, self.window.height),
                20,
            )
            upper_bound_s = pymunk.Segment(
                bounds_body,
                (self.window.width, self.window.height + 20),
                (0, self.window.height + 20),
                20,
            )
        ...
        if mouse_hover:
            ...
            self.space.add_collision_handler(
                self.COLLTYPE_BUTTON, self.COLLTYPE_MOUSE
            ).begin = self.on_mouse_hover
            self.space.add_collision_handler(
                self.COLLTYPE_BUTTON, self.COLLTYPE_MOUSE
            ).separate = self.on_mouse_unhover
            ###################################################################
...
class Menu(GameState):
    ...
    def __init__(self, batch, space, window):
        ...
        self.terrain = Terrain(
            ...
            self.window,
            mid_height=150,
            end_coordinate=self.window.width + 200,
        ...
        self.obstacles = Obstacles(
            ...
            self.space,
            self.window,
            end_coordinate=self.window.width,
            frequency=40,
            amount=2,
            x_offset=1600,
            group=self.back,
        ...
        # buttons #############################################################
        self.game1_button = Button(
            self.batch,
            self.space,
            "game1_button",
            (self.window.width // 2 - 250, 700),
            resources.game1_button_img,
            60,
        )
        self.game2_button = Button(
            self.batch,
            self.space,
            "game2_button",
            (self.window.width // 2 - 100, 700),
            resources.game2_button_img,
            60,
        )
        self.game3_button = Button(
            self.batch,
            self.space,
            "game3_button",
            (self.window.width // 2 + 50, 700),
            resources.game3_button_img,
            60,
        )
        self.game1_hs_button = Button(
            self.batch,
            self.space,
            "game1_hs_button",
            (self.window.width // 2 + 200, 700),
            resources.game1_hs_button_img,
            60,
        )
        self.game2_hs_button = Button(
            self.batch,
            self.space,
            "game2_hs_button",
            (self.window.width // 2 + 350, 700),
            resources.game2_hs_button_img,
            60,
        )
        self.gravity_button = Button(
            self.batch,
            self.space,
            "gravity_button",
            (self.window.width // 2 + 450, 700),
            resources.gravity_button_img,
            30,
        )
        self.fullscreen_button = Button(
            ...
            self.space,
            "fullscreen_button",
            (self.window.width // 2 + 550, 700),
            resources.fullscreen_button_img,
            60,
        )
        #######################################################################
...
class Game1(GameState):
    ...
    def __init__(self, batch, space, window):
        ...
        self.obstacles = Obstacles(
            self.batch, self.space, self.window, amount=3, group=self.back
        )
        # left bound
        bounds_body = pymunk.Body(body_type=pymunk.Body.STATIC)
        bounds_body.position = (0, 0)
        left_bound_s = pymunk.Segment(
            bounds_body, (300 - 20, 0), (300 - 20, self.window.height), 20
        )
        left_bound_s.filter = pymunk.ShapeFilter(categories=0b0001000, mask=0b1110111)
        # buttons #############################################################
        self.menu_button = pyglet.sprite.Sprite(
            img=resources.menu_button_img,
            x=55,
        ...
        # sprites #############################################################
        # finish flag sprite
        self.finishflag_sprite = pyglet.sprite.Sprite(
            img=resources.finishflag_img,
            x=self.end_position,
            y=self.window.height // 2 - 100,
            batch=self.batch,
            group=self.back,
        ...
        # motor meter sprite
        self.motormeter_sprite = MeterSprite(
            self.batch, (self.window.width // 2 - 320, 50), group=self.midground
        )
        # speed meter sprite
        self.speedmeter_sprite = MeterSprite(
            self.batch, (self.window.width // 2 + 320, 50), group=self.midground
        )
        # goal sprite
        self.goalmeter_sprite = GoalSprite(
            self.batch,
            (self.window.width - 230, self.window.height - 80),
            group=self.background,
        )
        #######################################################################
    ...
    def on_mouse_press(self, x, y, button, modifier):
        if (
            ...
            < x
            < self.menu_button.x + self.menu_button.width // 2
            and self.menu_button.y - self.menu_button.height // 2
            < y
    ...
    def update(self):
        ...
        if not self.ENDGAME and (self.tank1.lives == 0 or score < 0):
            ...
            self.restart_button = Button(
                ...
                self.space,
                "restart_button",
                (self.tank1.position[0], self.tank1.position[1] + 130),
                resources.restart_button_img,
                60,
            )
            ###################################################################
        ...
        self.motormeter_sprite.update(
            abs(self.tank1.wheels[0].body.angular_velocity) / 130
        )
...
class Game2(GameState):
    ...
    def update(self):
        ...
        self.finishflag_sprite.update(x=self.end_position - offset)
        self.motormeter_sprite.update(
            abs(self.motorbike.wheels[0].body.angular_velocity) / 35
        )
        self.speedmeter_sprite.update(
            abs(self.motorbike.chassis.body.velocity.x) / 1100
        )
...
class Game3(GameState):
    ...
    def __init__(self, batch, space, window):
        ...
        bounds_filter = pymunk.ShapeFilter(categories=0b0001000, mask=0b1110111)
        left_bound_s = pymunk.Segment(
            bounds_body, (-20, 0), (-20, self.window.height + ceiling), 20
        )
        right_bound_s = pymunk.Segment(
            bounds_body,
            (self.window.width + 20, 0),
            (self.window.width + 20, self.window.height + ceiling),
            20,
        )
        upper_bound_s = pymunk.Segment(
            bounds_body,
            (self.window.width, self.window.height + 20 + ceiling),
            (0, self.window.height + 20 + ceiling),
            20,
        )
        bounds_shapes.extend((left_bound_s, right_bound_s, upper_bound_s))
        for shape in bounds_shapes:
        ...
        self.space.add(bounds_body, *bounds_shapes)
        self.ball = None
        self.floor = SymmetricTerrain(
            self.batch, self.space, self.window, group=self.background
        )
        ...
        self.net_sprite = pyglet.sprite.Sprite(
            ...
            x=net_body.position.x,
            y=net_body.position.y,
            batch=self.batch,
            group=self.back,
        )
        ...
        self.player2 = VbVehicle(
            self.batch,
            self.space,
            self.window,
            ((window.width // 4) * 3, 250),
            side="right",
            group=self.background,
        )
        ...
        self.p1score_label = pyglet.text.Label(
            "2",
            # font_name='Times New Roman',
            font_size=56,
        ...
        self.p2score_label = pyglet.text.Label(
            "1",
            # font_name='Times New Roman',
            font_size=56,
            x=self.window.width // 2 + 50,
        ...
        # sprites #############################################################
        self.motormeter1_sprite = MeterSprite(
            self.batch, (110, 50), scale=0.8, group=self.midground
        )
        self.motormeter2_sprite = MeterSprite(
            self.batch, (self.window.width - 110, 50), scale=0.8, group=self.midground
        )
        ...
        #######################################################################
        self.space.add_collision_handler(
            self.COLLTYPE_VB_BALL, self.COLLTYPE_VB_PLAYER1
        ).begin = self.player2_scored
        self.space.add_collision_handler(
            self.COLLTYPE_VB_BALL, self.COLLTYPE_VB_PLAYER2
        ).begin = self.player1_scored
        self.window.push_handlers(*self.event_handlers)
    ...
    def player1_scored(self, arbiter, space, data):
        ...
        if self.player1.score == 7:
            ...
            pyglet.clock.schedule_once(
                self.dialog, 1, message="Dropping ball for player 2"
            )
    ...
    def player2_scored(self, arbiter, space, data):
        ...
        if self.player2.score == 7:
            ...
            pyglet.clock.schedule_once(
                self.dialog, 1, message="Dropping ball for player 1"
            )
            pyglet.clock.schedule_once(self.dialog, 2, message=2)
    ...
    def spawn_ball(self, dt, side):
        ...
        self.ball = GameBall(
            self.batch,
            self.space,
            ((self.window.width // 4) * offset, 700),
            70,
            group=self.front,
        )
    ...
    def update(self):
        ...
        # update sprites
        self.motormeter1_sprite.update(
            abs(
                min(
                    self.player1.wheels[0].body.angular_velocity,
                    self.player1.wheels[1].body.angular_velocity,
                )
            )
            / 37
        )
        self.motormeter2_sprite.update(
            abs(
                min(
                    self.player2.wheels[0].body.angular_velocity,
                    self.player2.wheels[0].body.angular_velocity,
                )
            )
            / 37
        ...
        if not (
            self.keys[pyglet.window.key.RIGHT] or self.keys[pyglet.window.key.LEFT]
        ):
            self.player2.stop()
...
class HighScore(GameState):
    ...
    def __init__(self, batch, space, window, *args, **kwargs):
        ...
        # scrolling hs ########################################################
        self.score_scroll = ScrollingText(
            self.batch,
            (325, 100 - 70),
            290 - 210,
            300,
        ...
        self.name_scroll = ScrollingText(
            self.batch,
            (325 + 100, 100 - 70),
            290 + 200,
            300,
        ...
        #######################################################################
        self.event_handlers.extend((self.on_mouse_press,))
...
class Endgame(GameState):
    ...
    def __init__(self, batch, space, window, *args, **kwargs):
        ...
        self.score = (
            round(self.score, 1) if isinstance(self.score, float) else self.score
        )
        self.score_label = pyglet.text.Label(
        ...
        self.userinput = TextInput(
            self.batch, (30, 100), 440, 26, group=self.foreground
        )
        #######################################################################
    ...
    def __name_entered(self):
        ...
        self.window.push_handlers(
            self.on_mouse_press, self.on_key_press, self.on_mouse_motion
        )
        self.update_hs_text()
